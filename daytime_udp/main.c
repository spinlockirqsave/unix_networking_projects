...
udp_server( const char *host, const char *serv, socklen_t *addrlenp)
{
	...
	if ( ( n = getaddrinfo( host, serv, &hints, &res)) != 0)
		err_quit( "udp_server getaddrinfo error for %s, %s: %s",
	...
	if ( res == NULL)	/* errno from final socket() or bind() */
		err_sys("udp_server (res==NULL) error for %s, %s", host, serv);
...
udp_connect( const char *host, const char *serv)
{
	...
	if ( ( n = getaddrinfo( host, serv, &hints, &res)) != 0)
		err_quit( "udp_connect getaddrinfo error for %s, %s: %s",
				 host, serv, gai_strerror(n));
	ressave = res;
	...
	if ( res == NULL)	/* errno set from final connect() */
		err_sys( "udp_connect (res==NULL) error for %s, %s", host, serv);
...
udp_client( const char *host, const char *serv, SA **saptr, socklen_t *lenp)
{
	...
	if ( ( n = getaddrinfo( host, serv, &hints, &res)) != 0)
		err_quit( "udp_client getaddrinfo error for %s, %s: %s",
				 host, serv, gai_strerror(n));
	ressave = res;
	...
	if ( res == NULL)	/* errno set from final socket() */
		err_sys( "udp_client (res==NULL) error for %s, %s", host, serv);
...
tcp_listen( const char *host, const char *serv, socklen_t *addrlenp)
{
	...
	if ( ( n = getaddrinfo( host, serv, &hints, &res)) != 0)
		err_quit( "tcp_listen getaddrinfo error for %s, %s: %s",
				 host, serv, gai_strerror(n));
	ressave = res;
	...
	if ( res == NULL)	/* errno from final socket() or bind() */
		err_sys( "tcp_listen (res==NULL) error for %s, %s", host, serv);
